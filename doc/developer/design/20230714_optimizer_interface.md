...
currently constructs this pipeline implicitly using at-hoc calls of relatively
...
1. Immediately solve long-standing issues with the optimizer pipeline. While the
   ...
   introdcing the notion of a shared context is part of the scope, threading it
   through is is not.
...
/// can passed to the optimizer struct and the `Self::To` types.
///
/// The `'s: 'ctx` bound in the `optimize` method call ensures that an optimizer
/// instance can run an optimization stage that producees a `Self::To` with
/// `&'ctx` references.
...
pub struct OptimizeMaterializedView {
    ...
    /// A human-readable exposed internally (useful for debugging).
    debug_name: String,
    // Optimizer flags.
...
1. Duplicate the `Catalog` code that is handling the "bootstrap `T`", "sequence
   `T`", and "explain `T`".
   - Mark the old code as "deprecated", referencing this
   section of the current design doc. This should prevent other engineers from
   making changes to only one of the two methods while they co-exist in `main`.
   - Dispatch between the old and the cloned version code based on the feature
   flag value.
1. Modify the cloned version of the code to use the new `Optimize` API. This
   should produce a clean diff between the old and the new version.
...
for a week or two, we can remove the old code paths.
