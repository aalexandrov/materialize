...
The supported degrees of freedom are described below.

### Explainee types

The following three explainee subject types are available.

Explainee type | Description
------|-----
...
Users can select between `JSON` and `TEXT` output format.

Output type | Description
------|-----
**TEXT** | The explanation output will be UTF-8 text.
**JSON** | The explanation output will a JSON object.

### Explained stage

This determines the query optimization stage at which the plan snapshot will be taken.
See the [Query Compilation Pipeline](#query-compilation-pipeline) section for details.
...
**join_impls** | Render details about the implementation strategy of physically optimized MIR `Join` nodes.
...
Sub-graphs consumed more than once are represented as common table expressions.
...
columns assigned to `Map` operators, it might be useful to request [the `arity` output modifier](http://localhost:1313/sql/explain/#output-modifiers).

Each operator can also be annotated with additional metadata. Details are shown by default in
the `EXPLAIN PHYSICAL PLAN` output, but are hidden elsewhere. In `EXPLAIN OPTIMIZDED PLAN`, details
about the implementation in the `Join` operator can be requested with [the `join_impls` output modifier](http://localhost:1313/sql/explain/#output-modifiers).
...
**Filter** | Remove rows of the input for which some scalar predicates return false. | `Filter (#20 < #21)`
**~Join** | Perform one of `INNER` / `LEFT` / `RIGHT` / `FULL OUTER` / `CROSS` join on the two inputs, using the given predicate. | `InnerJoin (#3 = #5)`.
