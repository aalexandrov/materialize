...
impl Coordinator {
    ...
    fn sequence_explain_plan(
        ...
        {
            if guard {
                match mz_ore::panic::catch_unwind(AssertUnwindSafe(f)) {
        ...
        let pipeline_result = optimizer_trace.collect_trace(|| -> Result<_, AdapterError> {
            ...
            // Execute the `optimize/raw` stage.
            ...
            // Calculate indexes used by the dataflow atthis point
            ...
            // when rendering explanations for the varous trace entries.
