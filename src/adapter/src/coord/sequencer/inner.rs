...
impl Coordinator {
    ...
    pub(super) async fn sequence_create_materialized_view(
        ...
        let optimized_plan = builder.build_materialized_view(
            id,
            internal_view_id,
            debug_name,
            &optimized_expr,
        ...
        match self
            .catalog_transact_with(Some(session), ops, |txn| {
                ...
                Ok(mv.optimized_plan.as_ref().expect("optimized_plan").clone())
            })
            .await
        {
            Ok(mut df) => {
    ...
    fn explain_materialized_view(
        ...
        if !matches!(stage, ExplainStage::OptimizedPlan) {
            coord_bail!(
                "cannot EXPLAIN {} FOR MATERIALIZED VIEW\n\nonly {} is supported",
                stage,
        ...
        let CatalogItem::MaterializedView(mv) = self.catalog().get_entry(&id).item() else {
            // This is currently asserted in the planner.
            // However, this assumtion might change when we make changes for #18089.
            unreachable!()
        };

        let Some(mut optimized_plan) = mv.optimized_plan.clone() else {
