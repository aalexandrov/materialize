...
//! that optimzie plans using the new optimizer API. The `sequence_plan` method
//! in the parent module will delegate to these methods only if the
//! `enable_unified_optimizer_api` feature flag is on. Once we have gained
...
impl Coordinator {
    /// Mirros `Coordinator::sequence_create_materialized_view`.
    #[tracing::instrument(level = "debug", skip(self))]
    pub(super) async fn sequence_create_materialized_view1(
        &mut self,
        session: &mut Session,
    ...
    /// `explain_query_optimizer_pipeline`, but it is passed in case changes to
    /// this function require it.
    #[tracing::instrument(target = "optimizer", level = "trace", name = "optimize", skip_all)]
    pub(crate) async fn explain_create_materialized_view_optimizer_pipeline1(
        &mut self,
