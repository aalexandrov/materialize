...
impl<'ctx> GlobalMirPlan<Unresolved<'ctx>> {
    ...
    /// the next stage.
    pub fn into_timestamp_plan(self, ctx: TimestampContext<Timestamp>) -> GlobalMirPlan<Resolved> {
        GlobalMirPlan {
...
impl<'ctx> Optimize<'ctx, GlobalMirPlan<Resolved>> for OptimizePeek<'ctx> {
    ...
    fn optimize<'s: 'ctx>(
        ...
        match create_fast_path_plan(
            ...
            self.select_id,
            self.finishing,
            self.flags.persist_fast_path_limit,
        )? {
            Some(plan) => {
            ...
            None => {
                ...
                let df_desc = Plan::finalize_dataflow(
                    ...
                    self.flags.enable_monotonic_oneshot_selects,
                )
