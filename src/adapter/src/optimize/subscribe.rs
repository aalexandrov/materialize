...
impl<'ctx> Optimize<'ctx, GlobalMirPlan<Resolved>> for OptimizeSubscribe<'ctx> {
    ...
    fn optimize<'s: 'ctx>(
        ...
        assert!(
            PartialOrder::less_equal(&as_of, &Antichain::from_elem(self.as_of)),
            "Dataflow subscribe-{} requested as_of ({:?}) not >= since ({:?})",
            self.subscribe_id,
            self.as_of,
            as_of
        );
        ...
        df_desc.set_as_of(Antichain::from_elem(self.as_of));
        ...
        // `up_to`s.
        //
        // TODO: Figure out if this is always true or always false for subscribe
        // dataflows.
