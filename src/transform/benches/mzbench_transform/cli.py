...
class Arg:
    scenario: Scenario = typer.Argument(
        ...,
...
class Opt:
    ...
    samples: int = typer.Option(
        11,
        help="Samples per query."
    )

    print_results: bool = typer.Option(
        False,
        help="Print the experiment results."
    )
...
def init(
    ...
    try:
        ...
        db.execute_all(
            statements=sql.parse_from_file(sql.schema_path(scenario))
        )
...
def run(
    ...
    try:
        ...
        df = pd.DataFrame(data={
            ...
            for query in [
                sql.Query(query)
                for query in sql.parse_from_file(sql.workload_path(scenario))
            ]
        })
...
def compare(
    ...
    try:
        ...
        quot_df['Avg'] = quot_df.mean(axis=1)
