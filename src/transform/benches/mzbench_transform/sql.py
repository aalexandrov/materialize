...
class Query:
    ...
    def explain(self, timing: bool) -> str:
        ...
        return '\n'.join([
            f"EXPLAIN (TIMING {bool(timing)}) PLAN FOR",
            self.query
        ])
...
def schema_path(scenario: Scenario) -> Path:
    """Returns the path to the file containsing the schema for the given scenario."""
    return Path(resource_filename('mzbench_transform', f'schema/{scenario}.sql'))


def workload_path(scenario: Scenario) -> Path:
    """Returns the path to the file containsing the queries for the given scenario."""
    return Path(resource_filename('mzbench_transform', f'workload/{scenario}.sql'))
